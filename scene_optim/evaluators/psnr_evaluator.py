...
class PSNREvaluator:
    ...
    def __init__(
        ...
        images: torch.Tensor,
        masks: torch.Tensor,
        cse_embedding_verts: torch.Tensor | None,
        image_size: int,
        device: str = "cuda",
    ):
        """
        ...
        self.masked_image = torch.where(
            self.masks_bool.unsqueeze(-1).expand(-1, -1, -1, 3),
            images,
    ...
    def evaluate(
        ...
        # torch.save((rendered_colors, rendered_masks), 'sample_rendered_colors.pt')
        ...
        for i in range(BATCH):
            # ->(1,3,NPTS)
            masked_gt = self.masked_image[X_ind[i]][
                self.masks_bool[X_ind[i]]
            ].unsqueeze(0)

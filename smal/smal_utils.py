...
def get_global_transf_from_kinematic_transf(
    ...
    results = [
        get_transf_from_RT(rotation_joints[:, 0], position_joints[:, 0].unsqueeze(-1))
    ]
    ...
    for i in range(1, parent.shape[0]):
        ...
        try:
            ...
            scale_parent_i_inv = torch.max(
                scale_joints[:, parent[i]],
                0.01 * torch.eye(3, device=device)[None, :, :],
            )

        # Translation ->(BATCH, 3, 1)
        position_rel_i = (
            position_joints[:, i] - position_joints[:, parent[i]]
        ).unsqueeze(-1)
        # Rotation ->(BATCH, 3, 3)
        ...
        transf_i = get_transf_from_RT(
            scale_parent_i_inv @ rot_i @ scale_i, position_rel_i
        )
    ...
    # ->(BATCH, N_JOINTS, 4, 1)
    Js_w0 = torch.cat(
        [position_joints.unsqueeze(-1), torch.zeros((N, 35, 1, 1), device=device)], 2
    )

...
def train_test_split_frames(
    X_ind: torch.Tensor, train_n: int = 15, test_n: int = 5
    ...
    X_train = X_ind[
        torch.tensor(train_list, dtype=torch.int32).reshape((-1,)).to(X_ind.device)
    ]
    X_test = X_ind[
        torch.tensor(test_list, dtype=torch.int32).reshape((-1,)).to(X_ind.device)
    ]

...
class TextureCSEFixed(nn.Module):
    ...
    def __init__(
        self, verts_colors: torch.Tensor, verts_cse_embedding: torch.Tensor, device: str
    ):
        """
        Args:
        ...
        self.verts_colors = torch.nn.Parameter(
            verts_colors.to(device), requires_grad=False
        )
        self.verts_cse_embedding = torch.nn.Parameter(
            verts_cse_embedding.to(device), requires_grad=False
        )
    ...
    def render_cse_embeddings(self, cse_embedding: torch.Tensor) -> torch.Tensor:
        ...
        closest_verts = get_closest_vertices_matrix(
            cse_embedding, self.verts_cse_embedding
        )
        # Return the color
    ...
    def render_images(
        ...
        if (vertices is None) or (faces is None):
            with torch.no_grad():
                vertices, _, faces = compute_pose(
                    smal, pose_model, X_ind, X_ts, dino_feature
                )

        # Render cse image
        rendered_cse, rendered_cse_mask = renderer.render_cse(
            vertices, faces, cameras, cse_embedding_verts
        )
        ...
        # Get the closest vertices
        closest_verts = get_closest_vertices_matrix(
            rendered_cse.reshape(bs * H * W, D_CSE), self.verts_cse_embedding
        ).reshape(bs, H, W, 3)

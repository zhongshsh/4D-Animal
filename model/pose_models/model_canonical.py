...
def rot_x(angle_rad: float) -> torch.Tensor:
    ...
    return torch.tensor(
        [
            [1.0, 0.0, 0.0],
            [0.0, torch.cos(tangle_rad), -torch.sin(tangle_rad)],
            [0.0, torch.sin(tangle_rad), torch.cos(tangle_rad)],
        ]
    )
...
def rot_y(angle_rad: float) -> torch.Tensor:
    ...
    return torch.tensor(
        [
            [torch.cos(tangle_rad), 0.0, torch.sin(tangle_rad)],
            [0.0, 1.0, 0.0],
            [-torch.sin(tangle_rad), 0.0, torch.cos(tangle_rad)],
        ]
    )
...
def rot_z(angle_rad: float) -> torch.Tensor:
    ...
    return torch.tensor(
        [
            [torch.cos(tangle_rad), -torch.sin(tangle_rad), 0.0],
            [torch.sin(tangle_rad), torch.cos(tangle_rad), 0.0],
            [0.0, 0.0, 1.0],
        ]
    )


def get_original_orient(
    N: int, front_orient: bool, mode_6d: bool = False
) -> torch.Tensor:
    """
    Returns:
    ...
    if front_orient:
        original_orient_3_3 = (
            (rot_z(0) @ rot_y(-np.pi / 2) @ rot_x(-np.pi / 2))
            .repeat((N, 1, 1, 1))
            .to(torch.float32)
        )
    else:
        original_orient_3_3 = (
            torch.eye(3).reshape((1, 1, 3, 3)).repeat((N, 1, 1, 1)).to(torch.float32)
        )
...
def get_original_pose(N, N_joints: int = 34, mode_6d: bool = False) -> torch.Tensor:
    ...
    """
    original_pose_3_3 = (
        torch.eye(3).reshape((1, 1, 3, 3)).repeat((N, N_joints, 1, 1)).to(torch.float32)
    )
    if mode_6d:
...
class PoseCanonical(PoseBase):
    ...
    def __init__(
        ...
        self.betas = _safe_parameter_init(
            betas, (1, 30), requires_grad=req_grad, init="zeros"
        )
        self.betas_limbs = _safe_parameter_init(
            betas_limbs, (1, 7), requires_grad=req_grad, init="zeros"
        )
        self.vertices_off = _safe_parameter_init(
            vert_off, (1, 5901), requires_grad=req_grad, init="zeros"
        )
    ...
    def compute_pose(
        self, X_ind: torch.Tensor, X_ts: torch.Tensor, **kwargs
    ) -> torch.Tensor:
    ...
    def compute_global(
        ...
        return torch.cat(
            [
                self.compute_transl(X_ind, X_ts),
                self.compute_orient(X_ind, X_ts).reshape(X_ind.shape[0], -1),
                self.compute_pose(X_ind, X_ts).reshape(X_ind.shape[0], -1),
            ],
            dim=-1,
        )

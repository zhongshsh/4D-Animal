...
import util.align_cam_utils as aligncamutil


def get_moving_cameras_from_PNP_archive(
    sequence_index: str,
    frame_limit: int,
    cache_path: str,
    cameras_original: PerspectiveCameras,
) -> PerspectiveCameras:
    """
    ...
    pnp_preprocess_file_path = os.path.join(
        cache_path, f"{sequence_index}_{frame_limit}_pnp_R_T.pk"
    )

    print(pnp_preprocess_file_path)
    assert os.path.isfile(pnp_preprocess_file_path), (
    ...
    valid_indices, R_pnp, T_pnp = (
        pnp_solution["valid_indices_pnp"],
        pnp_solution["R_PNP"],
        pnp_solution["T_PNP"],
    )

    return align_cameras_from_PNP_solution(
        cameras_original, R_pnp, T_pnp, valid_ind=None
    )


def align_cameras_from_PNP_solution(
    cameras_original: PerspectiveCameras,
    R_pnp: torch.Tensor,
    T_pnp: torch.Tensor,
    valid_ind=None,
    ...
    _, (align_t_R, align_t_T, align_t_s) = aligncamutil.corresponding_cameras_alignment(
        cameras_src=cameras_original[valid_ind],
        cameras_tgt=cameras_pnp[valid_ind],
        mode=align_mode,
        estimate_scale=estimate_scale,
        return_solution=True,
    )
    cameras_aligned = aligncamutil.align_cameras_from_solution(
        cameras_original, align_t_R, align_t_T, align_t_s
    )

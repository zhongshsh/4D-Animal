...
def resize_torch(
    X: torch.Tensor, size: tuple[int, int], mode="bilinear"
) -> torch.Tensor:
    """
    Args:
    ...
    """
    if type(X) == list:
        return [resize_torch(x, size) for x in X]
    if len(X.shape) == 3:
        return F.interpolate(
            X.permute((2, 0, 1)).unsqueeze(0), (size[0], size[1]), mode=mode
        ).permute((0, 2, 3, 1))[0]
    else:
        return F.interpolate(
            X.permute((0, 3, 1, 2)), (size[0], size[1]), mode=mode
        ).permute((0, 2, 3, 1))


def resize_torch_bool(
    X: torch.Tensor, size: tuple[int, int], alpha: float = 0.5, mode: str = "bilinear"
) -> torch.Tensor:
    """
    Args:
    ...
    if len(X.shape) == 3:
        return (
            F.interpolate(
                X.type(torch.float32).permute((2, 0, 1)).unsqueeze(0),
                (size[0], size[1]),
                mode=mode,
            ).permute((0, 2, 3, 1))[0]
            >= alpha
        ...
        return (
            F.interpolate(
                X.type(torch.float32).permute((0, 3, 1, 2)),
                (size[0], size[1]),
                mode=mode,
            ).permute((0, 2, 3, 1))
            >= alpha
...
def resize_np(
    X: np.ndarray, size: tuple[int, int], mode: str = "bilinear"
) -> np.ndarray:
    """
    Args:
    ...
    if len(X.shape) == 3:
        ...
        return (
            F.interpolate(
                torch.tensor(X).permute((0, 3, 1, 2)), (size[0], size[1]), mode=mode
            )
            .permute((0, 2, 3, 1))
            .numpy()
        )
...
def resize_np_uint8(X: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    ...
    if len(X.shape) == 3:
        ...
        return np.stack(
            [
                cv2.resize(X[i], (size[0], size[1]), cv2.INTER_AREA)
...
def normalize_torch(
    embeddings: torch.Tensor, dim: int = 1, epsilon: float = 1e-6
) -> torch.Tensor:
    """
    ...
    return embeddings / torch.clamp(
        embeddings.norm(p=None, dim=dim, keepdim=True), min=epsilon
    )


def normalize_numpy(
    embeddings: np.ndarray, dim: int = 1, epsilon: float = 1e-6
) -> np.ndarray:
    """

...
def get_lbo_from_name(
    mesh_name: str, query_folder: str
...
def get_mapping_from_name(
    mesh_source_name: str, mesh_destination_name: str, query_folder: str
) -> np.ndarray:
    """
    Args:
    ...
    mapping_filepath = os.path.join(
        query_folder, f"lbo_{mesh_source_name}_to_{mesh_destination_name}.pk"
    )
...
def map_X_from_source_to_destination(
    X_A: np.ndarray,
    lbo_areas_A: np.ndarray,
    lbo_basis_A: np.ndarray,
    lbo_areas_B: np.ndarray,
    lbo_basis_B: np.ndarray,
    C_A_to_B: np.ndarray,
) -> np.ndarray:
    """
...
def get_closest_vertices_matrix(
    X: torch.Tensor, Y: torch.Tensor, size_chunk: int = 10_000
) -> torch.Tensor:
    """
    ...
    Return:
        - output (M,) TORCHINT64
    """
    closest_vertices_X_to_Y = torch.zeros(
    ...
    for chunk in range((size_X - 1) // size_chunk + 1):
        ...
        closest_vertices_X_to_Y[size_chunk * chunk : size_chunk * (chunk + 1)] = (
            torch.argmin(distance_X_chunk_to_y, dim=1).type(torch.int64)
        )

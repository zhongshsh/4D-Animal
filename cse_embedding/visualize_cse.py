...
def visualize_cse_maps(
    ...
    X_texture = torch.tensor(templutil.get_mesh_2d_embedding_from_name("cse")).type(
        torch.float32
    )
    # Attach CSE vertex-embedding to vertex-color
    texture_model = TextureCSEFixed(
        verts_colors=X_texture / 255.0,
        verts_cse_embedding=cse_embedding_mesh,
        device=device,
    )
    rendered_cse_maps = render_cse_maps_from_texture_model(
        texture_model, cse_maps, cse_masks, valid_indices, images, device
    )
...
def render_cse_maps_from_texture_model(
    ...
    for j in range(N_frames):
        ...
        if j in valid_indices:
            rendered_image = texture_model.render_cse_embeddings(
                cse_maps[j].reshape((-1, D_CSE))
            ).reshape((H_img, W_img, 3))
            rendered_image = (rendered_image * 255).cpu().numpy().astype(np.uint8)
            rendered_cse_maps[j] = viz.blend_images(
                image_npy, rendered_image, 0.3, cse_masks_npy[j]
            )
        else:

...
def get_edm_X_mesh(
    X: torch.Tensor, mesh_embedding: torch.Tensor, temperature: float = 1 / 100_000
...
def get_cse_keypoint_cycle_masks(
    cse_images: torch.Tensor,
    cse_masks: torch.Tensor,
    cse_mesh: torch.Tensor,
    temperature: float = 1 / 100_000,
) -> torch.Tensor:
    """
    ...
    for i in range(BATCH):
        ...
        P_img_to_mesh, P_mesh_to_img = get_edm_X_mesh(
            cse_images[i][mask_i].reshape(-1, D_cse), cse_mesh, temperature
        )
